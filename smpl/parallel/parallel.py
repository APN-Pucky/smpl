...
def par(f,*args,**kwargs):
    ...
    """
    return res([calc(f,*[args[k][i] for k in enumerate(args)],**{k:v[i] for k,v in kwargs.items()}) for i in enumerate(args[0] if len(args)>0 else len(next(iter(kwargs.values()))))])
